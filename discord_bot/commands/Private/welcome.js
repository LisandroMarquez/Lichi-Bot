...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName("welcoming")
    .setDescription("Envía el welcome Embed para testeo")
  ...
  async execute(interaction) {
    ...
    const ctx = canvas.getContext("2d");
    const background = await Canvas.loadImage("../../src/a.jpg");
    ...
    ctx.font = applyText(
      canvas,
      `Bienvenido/a ${interaction.user.username} al servidor Estallados`
    );

    // Center text
    ctx.fillText(
      `Bienvenido/a ${interaction.user.username} al servidor de Estallados`,
      535,
      385
    );
    ctx.beginPath();
    ...
    const avatar = await Canvas.loadImage(
      interaction.user.displayAvatarURL({ size: 1024, extension: "png" })
    );
    ...
    const channel = interaction.guild.channels.cache.get("1100792651768406036"); // Change channel with the channel ID to send Welcome Embeds
